...
const LoginForm = (props: Props) => {
    const {hideClose = false, notAccountHandle, openHomePage, innerClass = "", hideHandler, source = ""} = props;
        ...
        <div className={`background-gradient p-0.5 rounded-md h-max ${innerClass} ${source==="homepage" ? 'parent' : ''} ${animate ? 'animateAdd' : ''}`}>
            <div className="rounded-md px-6 py-10 bg-neutral-900 relative">
                ...
                <div className='flex flex-col'>
                    <span className='text-center text-gray-500 uppercase'>Welcome Back</span>
                    <h4 className='text-white text-center m-2 text-lg'>Log into your account</h4>
                </div>
                <form className='form mt-10'>
                    <div className='flex flex-col'>
                        <label htmlFor="email" className="text-gray-400 text-sm">Email or Username</label>
                        <input className='mt-2.5 p-3 rounded-md bg-transparent border border-neutral-800 text-white placeholder:text-neutral-600' name='email' type='text' placeholder='Enter your email or username' />
                    </div>
                    <div className='flex flex-col mt-4'>
                        <div className='flex justify-between'>
                            <label htmlFor="password" className="text-gray-400 text-sm">Password</label>
                            <span className="text-gray-400 text-xs cursor-pointer">Forgot password?</span>
                        </div>
                        <div className="relative w-full">
                            <input className=' w-full mt-2.5 p-3 pe-10 rounded-md bg-transparent border border-neutral-800 text-white placeholder:text-neutral-600' name='password' type={type} placeholder='Enter your password' />
                    ...
                    <button className='mt-5 rounded-md bg-blue-500 text-white w-full py-3 text-center' onClick={openHomePage}>Login now</button>
                    <div>
                        <p className='text-neutral-500 mt-2.5 text-sm flex'>Not registered yet?
